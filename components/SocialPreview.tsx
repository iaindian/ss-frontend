...
export function SocialPreviewDialog({
  open,
  onOpenChange,
  images,
  title,
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft")
        setIdx((p) => (p - 1 + pics.length) % pics.length);
          ...
          <div className="rounded-2xl border border-border bg-background p-4 text-sm opacity-80">
            ...
            {tab === "instagram" ? (
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  Square (1:1) = grid-friendly. Portrait (4:5) gets more screen
                  height.
                </li>
                <li>Keep core subject centered; avoid edge text.</li>
                <li>
                  Export: 1080×1080 (1:1), 1080×1350 (4:5), 1080×1920 (9:16).
                </li>
              </ul>
            ) : (
              <ul className="list-disc pl-4 space-y-1">
  ...
  return (
    <button
      ...
      className={`rounded-full px-3 py-1 text-xs ${
        active ? "bg-primary text-black" : "bg-muted"
      }`}
    >
      {label}
    </button>
